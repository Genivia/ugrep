...
#
#   If 'bits' is empty or '8', PCRE2 8-bit character support is checked only.
#   If 'bits' contains '16', PCRE2 8-bit and 16-bit character support are
#   checked. If 'bits' contains '32', PCRE2 8-bit and 32-bit character support
#   are checked. When 'bits' contains both '16' and '32', PCRE2 8-bit, 16-bit,
#   and 32-bit character support is checked.
#
#   If either the header file (pcre2.h), or the library (libpcre2-8) is not
#   found, or the specified PCRE2 character bit width is not supported, shell
#   commands 'action-if-not-found' is run. If 'action-if-not-found' is not
...
#   LDFLAGS, prepends '-lpcre2-8' to LIBS, and calls AC_DEFINE(HAVE_PCRE2). You
#   should use autoheader to include a definition for this symbol in a config.h

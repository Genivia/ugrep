...
complete -c ug -s b -l byte-offset -d 'The offset in bytes of a matched line is displayed in front of the respective matched line.'
...
complete -c ug -l depth -s 1 -s 2 -s 3 -s 9 -o '10' -o '11' -o '12' -d 'Restrict recursive searches from MIN to MAX directory levels deep, where -1 (--depth=1) searches the specified path without recursing into subdirectories.'
...
complete -c ug -s e -l regexp -d 'Specify a PATTERN used during the search of the input: an input line is selected if it matches any of the specified patterns.'
...
complete -c ug -s g -l glob -l iglob -d 'Search only files whose name matches the specified comma-separated list of GLOBS, same as --include=glob for each glob in GLOBS.'
...
complete -c ug -l include -d 'Search only files whose name matches GLOB using wildcard matching, same as -g GLOB.'
...
complete -c ug -s j -l smart-case -d 'Perform case insensitive matching like option -i, unless a pattern is specified with a literal ASCII upper case letter.'
...
complete -c ug -s k -l column-number -d 'The column number of a matched pattern is displayed in front of the respective matched line, starting at column 1.'
...
complete -c ug -s N -l neg-regexp -d 'Specify a negative PATTERN used during the search of the input: an input line is selected only if it matches the specified patterns unless it matches the negative PATTERN.'
...
complete -c ug -s O -l file-extension -d 'Search only files whose filename extensions match the specified comma-separated list of EXTENSIONS, same as -g *.ext for each'
complete -c ug -s o -l only-matching -d 'Output only the matching part of lines.'
complete -c ug -l only-line-number -d 'The line number of the matching line in the file is output without displaying the match.'

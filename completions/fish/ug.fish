...
complete -c ug -l from -d 'Read additional pathnames of files to search from FILE'
...
complete -c ug -l hidden -s '.' -d 'Search hidden files and directories (enabled by default in grep compatibility mode)'
...
complete -c ug -l null -s 0 -d 'Output a zero byte after the file name'
complete -c ug -l null-data -o '00' -d 'Input and output are treated as sequences of lines with each line terminated by a zero byte instead of a newline; effectively swaps NUL with LF in the input and the output'
...
complete -c ug -s t -l file-type -xa 'actionscript ada adoc asm asp aspx autoconf automake awk Awk basic batch bison c c3 c++ clojure coffeescript cpp crystal csharp css csv dart Dart delphi elisp elixir erlang faust fennel fortran gif Gif go groovy gsp haskell html jade janet java jpeg Jpeg js json jsp julia kotlin less lex lisp lua m4 make markdown matlab nim nix node Node objc objc++ ocaml org parrot pascal pdf Pdf perl Perl php Php png Png prolog python Python r racket rpm Rpm rst rtf Rtf ruby Ruby rust scala scheme seed7 shell Shell smalltalk sql svg swift tcl tex text tiff Tiff tt typescript vala verilog vhdl vim xml Xml yacc yaml zig ' -d 'Search only files associated with TYPES, a comma-separated list of file types'
...
complete -c ug -s Y -l empty -d 'Empty-matching patterns match all lines'
...
complete -c ug -l zmax -d 'When used with option -z or --decompress, searches the contents of compressed files and archives stored within archives by up to NUM expansion stages'

...
complete -c ugrep -l null -s 0 -d 'Output a zero byte after the file name'
complete -c ugrep -l null-data -o '00' -d 'Input and output are treated as sequences of lines with each line terminated by a zero byte instead of a newline; effectively swaps NUL with LF in the input and the output'
...
complete -c ugrep -l pretty -d 'When output is sent to the terminal, enables options --color, --heading, -n, --sort, --tree and -T when not explicitly disabled'
...
complete -c ugrep -s Y -l empty -d 'Empty-matching patterns match all lines'
...
complete -c ugrep -l zmax -d 'When used with option -z or --decompress, searches the contents of compressed files and archives stored within archives by up to NUM expansion stages'
